...
class TerzoConto_Movimenti_Service {
    ...
    public function create(array $data) {
        ...
        if (! $created) {
            ...
            return new WP_Error('movimento_create_failed', $error !== '' ? $error : __('Errore creazione movimento', 'terzoconto-rendiconto-ets'));
        }
    ...
    public function update(int $id, array $data) {
        $id = absint($id);
        if ($id <= 0) {
            return new WP_Error('invalid_id', __('ID non valido', 'terzoconto-rendiconto-ets'));
        }
        ...
        if (! $updated) {
            ...
            return new WP_Error('movimento_update_failed', $error !== '' ? $error : __('Errore aggiornamento movimento', 'terzoconto-rendiconto-ets'));
        }
    ...
    private function validate(array $data, int $movimento_id = 0) {
        if (($data['data_movimento'] ?? '') === '' || ! $this->is_valid_date((string) $data['data_movimento'])) {
            return new WP_Error('invalid_data_movimento', __('Inserisci una data movimento valida.', 'terzoconto-rendiconto-ets'));
        }

        if ((float) ($data['importo'] ?? 0.0) <= 0.0) {
            return new WP_Error('invalid_importo', __('Inserisci un importo maggiore di zero.', 'terzoconto-rendiconto-ets'));
        }

        if ((int) ($data['categoria_associazione_id'] ?? 0) <= 0) {
            return new WP_Error('invalid_categoria_associazione', __('Seleziona una categoria valida.', 'terzoconto-rendiconto-ets'));
        }

        if ((int) ($data['conto_id'] ?? 0) <= 0) {
            return new WP_Error('invalid_conto', __('Seleziona un conto valido.', 'terzoconto-rendiconto-ets'));
        }

        if (isset($data['tipo']) && $data['tipo'] !== '' && ! in_array((string) $data['tipo'], ['entrata', 'uscita'], true)) {
            return new WP_Error('invalid_tipo', __('Tipo movimento non valido.', 'terzoconto-rendiconto-ets'));
        }

        $raccolta_id = absint((int) ($data['raccolta_fondi_id'] ?? 0));
        if ($raccolta_id > 0 && ! $this->raccolte->is_open($raccolta_id)) {
            return new WP_Error('raccolta_chiusa', __('La raccolta fondi è chiusa.', 'terzoconto-rendiconto-ets'));
        }

        if ($movimento_id > 0) {
            $current = $this->movimenti->find_by_id($movimento_id);
            if (! is_array($current)) {
                return new WP_Error('movimento_not_found', __('Movimento non trovato.', 'terzoconto-rendiconto-ets'));
            }
            ...
            if ($current_year > 0 && $new_year !== $current_year) {
                return new WP_Error('invalid_anno_change', __("Non è possibile modificare l'anno di un movimento. Eliminare il movimento e crearne uno nuovo.", 'terzoconto-rendiconto-ets'));
            }
        }
